...
# verify we have all the info we need to deploy
if [ ! $GIT_DEPLOY_DIR ]; then
  echo "DEPLOY ERROR - environment variable not set: GIT_DEPLOY_DIR"
  exit 1
fi
if [ ! $GIT_DEPLOY_BRANCH ]; then
  echo "DEPLOY ERROR - environment variable not set: GIT_DEPLOY_BRANCH"
  exit 1
fi
if [ ! $GITHUB_TOKEN ]; then
  echo "DEPLOY ERROR - environment variable not set: GITHUB_TOKEN"
  exit 1
fi
if [ ! $GITHUB_USER ]; then
  echo "DEPLOY ERROR - environment variable not set: GITHUB_USER"
  exit 1
fi
if [ ! $GITHUB_REPO ]; then
  echo "DEPLOY ERROR - environment variable not set: GITHUB_REPO"
  exit 1
fi
